...
conversation_context = {
    ...
    "iterative_research_active": False,  # Track if iterative research mode is active
    "iterative_research_state": None,  # Store state of iterative research
}
...
# New function: perform web search using DuckDuckGo API
from googlesearch import search


def perform_web_search(query, max_results=5):
    """
    Perform a web search using Google Search API (googlesearch-python).
    Returns a list of result snippets (titles and URLs).
    """
    try:
...
# New function: iterative research loop inspired by IterDRAG
def iterative_research_loop(
    user_query, max_iterations=3, model="gemma3", system_prompt=None
):
    """
    Perform iterative research inspired by IterDRAG:
    1. Generate initial search query from user query
    2. Perform web search
    3. Summarize results
    4. Reflect on summary to identify knowledge gaps
    5. Generate new search query to address gaps
    6. Repeat for max_iterations
    7. Return final summary with citations separated

    The returned string includes the main answer first, followed by a clearly separated section with citations or bibliography.
    The TTS should only read the main answer part, not the citations.
    """
    logger.info(f"Starting iterative research for query: {user_query}")
    current_query = user_query
    accumulated_summary = ""
    accumulated_citations = ""
    for iteration in range(1, max_iterations + 1):
        logger.info(f"Iteration {iteration} with query: {current_query}")

        # Step 1: Generate search query prompt
        prompt_generate_query = f"""
You are an expert research assistant. Given the research topic or question below, generate a concise and effective web search query to find relevant information.

Research topic: {current_query}

Search query:"""

        search_query = ask_ollama(
            prompt_generate_query, model=model, system_prompt=system_prompt
        )
        search_query = search_query.strip().replace("\n", " ")
        logger.info(f"Generated search query: {search_query}")

        # Step 2: Perform web search using DuckDuckGo API
        search_results = perform_web_search(search_query, max_results=5)
        if not search_results:
            logger.warning("No search results found, stopping iteration.")
            break

        # Step 3: Summarize search results with citations
        combined_results = "\n".join(search_results)
        prompt_summarize = f"""
You are an expert research assistant. Summarize the following search results related to the research topic '{current_query}'. Keep the summary concise and informative. Provide citations or source references clearly separated from the main summary.
...
Summary with citations:"""
        ...
        summary_with_citations = ask_ollama(
            prompt_summarize, model=model, system_prompt=system_prompt
        )
        summary_with_citations = summary_with_citations.strip()
        logger.info(
            f"Summary of search results with citations: {summary_with_citations}"
        )

        # Separate main summary and citations by delimiter (e.g., 'Citations:' or 'Sources:')
        if "Citations:" in summary_with_citations:
            main_summary, citations = summary_with_citations.split("Citations:", 1)
        elif "Sources:" in summary_with_citations:
            main_summary, citations = summary_with_citations.split("Sources:", 1)
        else:
            main_summary = summary_with_citations
            citations = ""

        main_summary = main_summary.strip()
        citations = citations.strip()

        # Accumulate summary and citations separately
        accumulated_summary += f"\nIteration {iteration} summary:\n{main_summary}\n"
        if citations:
            accumulated_citations += (
                f"\nIteration {iteration} citations:\n{citations}\n"
            )

        # Step 4: Reflect on summary to identify knowledge gaps
        prompt_reflect = f"""
You are an expert research assistant. Given the summary below, identify any knowledge gaps or areas that require further research. Provide a brief list of questions or topics that should be explored next.

Summary:
{main_summary}

Knowledge gaps or next questions:"""

        reflection = ask_ollama(
            prompt_reflect, model=model, system_prompt=system_prompt
        )
        reflection = reflection.strip()
        logger.info(f"Reflection on summary: {reflection}")

        # Step 5: Generate new search query to address gaps
        prompt_new_query = f"""
You are an expert research assistant. Based on the knowledge gaps or questions below, generate a new concise web search query to find relevant information.

Knowledge gaps or questions:
{reflection}

New search query:"""

        new_query = ask_ollama(
            prompt_new_query, model=model, system_prompt=system_prompt
        )
        new_query = new_query.strip().replace("\n", " ")
        logger.info(f"Generated new search query: {new_query}")

        # Prepare for next iteration
        current_query = new_query

    # Step 7: Return accumulated summary with citations separated
    final_summary_prompt = f"""
You are an expert research assistant. Based on the iterative summaries below, produce a final comprehensive summary of the research topic '{user_query}'. Include citations to sources clearly separated from the main summary.
...
Final summary:"""
    ...
    final_summary_with_citations = ask_ollama(
        final_summary_prompt, model=model, system_prompt=system_prompt
    )
    final_summary_with_citations = final_summary_with_citations.strip()
    logger.info("Iterative research completed.")
    return final_summary_with_citations


# Modify chatbot_interface to detect iterative research mode and handle accordingly
def chatbot_interface(image, message, history):
    ...
    try:
        ...
        # Check if iterative research mode is active
        if conversation_context.get("iterative_research_active", False):
            # Continue iterative research
            final_summary = iterative_research_loop(
                message, max_iterations=3, model="gemma3", system_prompt=None
            )
            history.append({"role": "user", "content": message})
            history.append({"role": "assistant", "content": final_summary})
            speak_text(final_summary)
            # Reset iterative research mode
            conversation_context["iterative_research_active"] = False
            return history, "", history
        ...
        # Detect if user wants to start iterative research
        if message.lower().startswith("research:"):
            # Activate iterative research mode
            conversation_context["iterative_research_active"] = True
            research_query = message[len("research:") :].strip()
            final_summary = iterative_research_loop(
                research_query, max_iterations=3, model="gemma3", system_prompt=None
            )
            history.append({"role": "user", "content": message})
            history.append({"role": "assistant", "content": final_summary})
            speak_text(final_summary)
...
def ask_ollama_with_image(prompt, image_path, model="qwen2.5vl", system_prompt=None):
    """
...
def chatbot_interface(image, message, history, iterdrag_mode):
    global image_desc
    global conversation_context
    ...
    try:
        ...
        # If IterDRAG mode is active, run iterative research loop
        if iterdrag_mode:
            final_summary_with_citations = iterative_research_loop(
                message,
                max_iterations=3,
                model="gemma3",
                system_prompt=CONVERSATION_SYSTEM_PROMPT,
            )
            # Split main answer and citations
            if "Citations:" in final_summary_with_citations:
                main_answer, citations = final_summary_with_citations.split(
                    "Citations:", 1
                )
            elif "Sources:" in final_summary_with_citations:
                main_answer, citations = final_summary_with_citations.split(
                    "Sources:", 1
                )
            else:
                main_answer = final_summary_with_citations
                citations = ""
            ...
            history.append({"role": "user", "content": message})
            history.append({"role": "assistant", "content": main_answer})

            if citations:
                history.append({"role": "system", "content": f"Sources:\n{citations}"})

            speak_text(main_answer)
            return history, "", history
            ...
            try:
                with sr.Microphone() as source:
                    ...
                    try:
                        ...
                        # No speech detected within timeout, continue listening silently
                        continue
...
def toggle_voice_listener(active, history_state, iterdrag_mode):
    ...
    if active:
        ...
        def voice_command_callback(command):
            ...
            history_state, _, history_state = process_voice_command(
                command, None, history_state, iterdrag_mode
            )
            # Update UI - note: this requires Gradio's live updates feature
            # Removed chatbot.update(history_state) because Gradio Chatbot has no update method

        # Start listening
        if voice_listener.start_listening(voice_command_callback):
            notification = (
                "Voice assistant activated. Say 'Hey Assistant' to get my attention."
            )
